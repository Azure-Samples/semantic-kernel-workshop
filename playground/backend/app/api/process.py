...
from app.models.api_models import ChatProcessRequest, ChatResponse, ContentProcessRequest, ContentResponse
...
class UserInputStep(KernelProcessStep[UserInputState]):
    ...
    async def get_user_input(self, context: KernelProcessStepContext):
        ...
        await context.emit_event(process_event=ChatBotEvents.UserInputReceived, data=user_message)
...
class ChatBotResponseStep(KernelProcessStep[ChatBotState]):
    ...
    async def get_chat_response(self, context: KernelProcessStepContext, user_message: str, kernel: Kernel):
        ...
        try:
            ...
            self.state.chat_messages.append({"role": "assistant", "content": error_message})
            await context.emit_event(process_event=ChatBotEvents.AssistantResponseGenerated, data=error_message)
...
async def start_chat_process(request: ChatProcessRequest, background_tasks: BackgroundTasks):
    """
...
async def send_message(process_id: str, request: ChatProcessRequest):
    ...
    if user_message.lower() == "exit":
        return ChatResponse(
            ...
            chat_history=process_data["messages"] + [{"role": "assistant", "content": "Goodbye! Chat session ended."}]
        )
